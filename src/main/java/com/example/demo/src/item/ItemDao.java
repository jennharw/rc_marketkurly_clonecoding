...
public class ItemDao {
    ...
    public List<GetItemRes> getItems(){
        String getItemsQuery = "select I.id AS id , I.name AS name ,I.price AS price , I.discount_rate AS discount_rate,\n" +
                ...
                "FROM HOME_CATEGORY A LEFT JOIN SUB_CATEGORY SC on A.id = SC.category_id LEFT JOIN ITEMS I on SC.id = I.sub_id;";
...
// HOW 전체 조회 API
    ...
    public List<GetHowRes> getHows(){
        String getHowsQuery = "select A.category AS category,B.name AS name,B.price AS price\n" +
                "                ,B.items_img_url AS items_img_url,B.discount_rate AS discount_rate, B.member_discount_price AS member_discount_price\n" +
                "                ,P.present_bool AS present,C.coupon_bool AS coupon\n" +
                "                from HOME_CATEGORY A LEFT JOIN ITEMS B ON A.id = B.new_product_order_id\n" +
                "                LEFT JOIN ITEM_PRESENTS P on B.id = P.items_id\n" +
                "                LEFT JOIN ITEM_COUPONS C on B.id = C.items_id\n" +
                "                LEFT JOIN BASKETS B2 on B.id = B2.item_id\n" +
                "                WHERE B2.count >= 1 AND (B.coupon = C.coupon_bool OR P.present_bool = B.present)\n" +
                "                ORDER BY B2.count";
        return this.jdbcTemplate.query(getHowsQuery,
                (rs,rowNum) -> new GetHowRes(
                        ...
                        rs.getString("items_img_url"),
                        rs.getString("discount_rate"),
                        rs.getString("member_discount_price"),
                        rs.getInt("present"),
                        rs.getInt("coupon")
                )
        );
